@extends('layouts.app', ['title' => 'Edit Santri - Admin'])
...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-300">
    <div class="container mx-auto px-6 py-8">
        ...
        <div class="p-6 bg-white rounded-md shadow-md">
            <h2 class="text-lg text-gray-700 font-semibold capitalize">Edit Santri</h2>
            <hr class="mt-4">
            <form action="{{ route('admin.employment.update', $employment->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-6 mt-4">
                    ...
                    <div>
                        ...
                        <input class="form-input w-full mt-2 rounded-md bg-gray-200 focus:bg-white" type="text" name="name" value="{{ old('name', $employment->name) }}" placeholder="Nama Kantor">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input class="form-input w-full mt-2 rounded-md bg-gray-200 focus:bg-white" type="text" name="username" value="{{ old('username', $employment->username) }}" placeholder="Username">
                        @error('username')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                <div class="px-4 py-2">
                                    <p class="text-gray-600 text-sm">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select class="w-full border mt-2 bg-gray-200 focus:bg-white rounded px-3 py-2 outline-none" name="office_id">
                            @foreach($offices as $office)
                                <option class="py-1" value="{{ $office->id }}" @if($employment->office_id == $office->id) selected @endif>{{ $office->name }}</option>
                            @endforeach
                        </select>
                        @error('office_id')
                            <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                    ...
                    <div>
                        ...
                        <select class="w-full border mt-2 bg-gray-200 focus:bg-white rounded px-3 py-2 outline-none" name="jabatan">
